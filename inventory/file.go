...
func (f *fileClient) Delete(ctx context.Context, files []*ent.File, options *types.EntityProps) ([]*ent.Entity, StorageDiff, error) {
	...
	for i, group := range fileGroups {
		...
		paths, err := CollectHLSArtifactPaths(ctx, f.client, chunks[i])
		if err != nil {
			return nil, nil, err
		}
		if len(paths) > 0 {
			tx, ok := ctx.Value(TxCtx{}).(*Tx)
			if !ok {
				return nil, nil, fmt.Errorf("HLS deletion requires a transaction")
			}
			for tx.inherited {
				tx = tx.parent
			}
			tx.hlsCleanup = append(tx.hlsCleanup, paths...)
		}
