...
const (
	...
	QuerySubtitleName = "subtitle_name"
...
func BuildSubtitleURL(base *url.URL, path string, claims SubtitleURLClaims, secret string) (string, error) {
	if base == nil {
		return "", fmt.Errorf("missing source url base")
	}
	if strings.TrimSpace(secret) == "" {
	...
	q.Set(QueryFileID, strconv.Itoa(claims.FileID))
	q.Set(QueryEntityID, strconv.Itoa(claims.EntityID))
	q.Set(QuerySubtitleName, claims.SubtitleName)
	q.Set(QueryExpires, strconv.FormatInt(claims.Expires, 10))
	q.Set(QueryNonce, claims.Nonce)
	q.Set(QuerySignature, SignSubtitleURL(claims, secret))
	u.RawQuery = q.Encode()
	return u.String(), nil
}
...
func VerifySubtitleURL(taskID int, values url.Values, secret string, now time.Time) (SubtitleURLClaims, error) {
	claims := SubtitleURLClaims{TaskID: taskID}
	if strings.TrimSpace(secret) == "" {
		return claims, ErrMissingSecret
	}
	...
	claims.SubtitleName = strings.TrimSpace(values.Get(QuerySubtitleName))
	if claims.SubtitleName == "" {
		return claims, ErrInvalidToken
	}
	if claims.Expires, err = strconv.ParseInt(values.Get(QueryExpires), 10, 64); err != nil || claims.Expires <= 0 {
		return claims, ErrInvalidToken
	...
	expected := SignSubtitleURL(claims, secret)
	if subtle.ConstantTimeCompare([]byte(expected), []byte(values.Get(QuerySignature))) != 1 {
		return claims, ErrInvalidToken
	}
...
func SignSubtitleURL(claims SubtitleURLClaims, secret string) string {
	payload := fmt.Sprintf("%d\n%d\n%d\n%s\n%d\n%s", claims.TaskID, claims.FileID, claims.EntityID, claims.SubtitleName, claims.Expires, claims.Nonce)
	mac := hmac.New(sha256.New, []byte(secret))
	_, _ = mac.Write([]byte(payload))
	return base64.RawURLEncoding.EncodeToString(mac.Sum(nil))
}
